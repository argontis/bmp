...
# Replace the old static grid with the new dynamic block
# We use regex to find the block from <div... id="gallery-container"> down to the closing </div> before "Load More"
pattern = re.compile(r'<!-- Masonry/Grid Gallery -->\s*<div class="grid[^>]+id="gallery-container">.*?</div>\s*<!-- Load More Button -->', re.DOTALL)
content = pattern.sub(dynamic_gallery + '\n\n                        <!-- Load More Button -->', content)
