...
# Replace everything between <main> and </main> with a detail view placeholder.
# Wait, let's find the exact <main> tag.
match = re.search(r'(<main[^>]*>).*?(</main>)', template, re.DOTALL)
if match:
    header_part = template[:match.start(1) + len(match.group(1))]
    footer_part = template[match.start(2):]
    ...
    for article in articles:
        ...
        # In the original template, the hero section is also outside <main> (it's above it). We should probably just clear out anything between <nav> and <main>.
        # Actually, let's just make a cleaner HTML string combining the layout.
        
        # Let's remove the hero section which is inside the header or just before main.
        # Find where nav ends
        nav_match = re.search(r'(</nav>).*?(<main[^>]*>)', template, re.DOTALL)
        
        if nav_match:
            new_html = template[:nav_match.start(1) + len(nav_match.group(1))] + "\n<main class=\"pt-24\">\n" + main_content + footer_part
        else:
